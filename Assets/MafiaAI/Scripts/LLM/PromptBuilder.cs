...
namespace MafiaAI.LLM
{
    ...
    public static class PromptBuilder
    {
        ...
        {
            string task =
                "[지시] 지금은 낮 토론이다. 너 자신의 승리 조건과 성격에 맞춰 한 번 발언하라.\n" +
                "- 먼저 최근 대화에서 누가 이득을 봤는지, 누가 손해를 봤는지 생각하라.\n" +
                "- 할 수 있는 행동 후보: 누군가를 떠보기, 내 의심을 세우기, 나나 아군을 변호하기, 모순을 짚기, 투표 구도를 만들기, 아직 단서가 약하면 동선/목격자를 캐기.\n" +
                "- 첫날 초반에는 확신형 억까보다 '왜 그렇게 말했는지'와 '어디 있었는지'를 캐는 쪽이 자연스럽다.\n" +
                "- 말은 짧게. 1~2문장, 이름표 없이.";
        ...
        public static Prompt FreeTalk(GameState s, Player self)
        {
            string task =
                ...
                "- 아니라면 네 성격대로 판을 움직여라: 떠보기, 의심 쌓기, 변호, 역질문, 동선 요구, 투표선 흔들기 중 하나를 고른다.\n" +
                "- 아직 정보가 없으면 범인 단정보다 검증 질문이 더 강한 플레이다.\n" +
                "- 1~2문장, 이름표 없이.";
            return new Prompt(BuildSystem(s, self), BuildUser(s, self, task));
        }
        ...
        static string BuildSystem(GameState s, Player self)
        {
            ...
            sb.AppendLine("- 말하기 전에 속으로 고른다: 지금 내가 얻을 이득은 정보, 생존, 신뢰, 투표 구도, 시선 돌리기 중 무엇인가?");
            sb.AppendLine("- 초반에는 근거 없는 확신보다 동선, 목격자, 말 바뀜, 질문 의도, 침묵의 타이밍을 캐는 플레이가 강하다.");
            sb.AppendLine("- 중후반에는 누가 누구를 살렸는지, 누가 누구를 몰았는지, 직업 공개 타이밍과 조사 결과의 모순을 이용하라.");
            sb.AppendLine("- 직업 공개는 항상 손익이 있다. 경찰/의사는 살해 위험, 마피아는 사칭 기회, 시민은 정보 노출 위험을 계산하라.");
            sb.AppendLine("- 말투와 판단 방식은 네 페르소나를 따른다. 같은 상황도 캐릭터마다 다르게 읽고 다르게 말해라.");
            return sb.ToString();
        }
