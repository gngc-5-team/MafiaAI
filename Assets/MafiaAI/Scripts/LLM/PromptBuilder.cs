    ...
    public static class PromptBuilder
    {
        ...
        public static Prompt Discussion(GameState s, Player self)
        {
            string task =
                ...
                "- 근거가 있으면 생존자 한 명을 이름으로 찍고 이유를 말하라. 아직 단서가 부족하면 의심을 단정하지 말고 동선·목격자·방금 발언의 근거를 확인하라.\n" +
                "- 좋은 검증 소재: 알리바이, 이동 동선, 말 바뀜, 모순, 대답회피, 남을 몰아가는 타이밍\n" +
                "- 초반 무근거 직업 공개 요구는 주의하라. 너도 함부로 직업을 까라고 하지 마라.\n" +
        ...
        public static Prompt FreeTalk(GameState s, Player self)
        {
            string task =
                ...
                "- 그게 아니면 근거가 있는 사람만 찍어라: 말 바꾸기, 무근거 몰이, 침묵, 모순, 부적절한 타이밍의 직업공개요구 중 하나가 보일 때만 의심하라.\n" +
                "- 이미 나온 말을 반복하지 말고 다음 행동을 요구하라: 알리바이 말해, 목격자 말해, 방금 의심의 근거를 말해.\n" +
        ...
        public static Prompt React(GameState s, Player self, string speakerId, string statement)
        {
            string task =
                ...
                "- 판단 없는 되묻기는 금지하되, 근거가 부족한 초반에는 단정 대신 검증 질문으로 압박하라. 1~2문장, 이름표 없이.";
            return new Prompt(BuildSystem(s, self), BuildUser(s, self, task));
        }
        ...
        public static Prompt Rebuttal(GameState s, Player self, string presserId, string question)
        {
            string task =
                ...
                "- 의심이나 추궁이면 내 알리바이/입장을 짧게 말하고, 근거가 약하면 " + presserId + "의 질문 의도를 역으로 검증하라.\n" +
                "- 1~2문장, 이름표 없이.";
            return new Prompt(BuildSystem(s, self), BuildUser(s, self, task));
        }
        ...
        public static Prompt RoomQuestion(GameState s, Player self, Player target, string room, string localTranscript)
        {
            string task =
                ...
                "- 질문은 친절한 인터뷰가 아니라 검증이어야 한다. 단서 없이 범인 취급하지 말고, 답하지 않으면 어떤 부분이 의심되는지 말하라.\n" +
                "- 나쁜 질문: 무근거 직업 공개 요구, 무차별 증거 강요, 시작하자마자 확신형 몰이, '뭐 들었어?', '무슨 의미야?', '정말 궁금해서' 같은 느슨한 질문.\n" +
        ...
        static string BuildSystem(GameState s, Player self)
        {
            ...
            sb.AppendLine("[규칙] 너는 사람인 척 연기하는 마피아 게임 참가자다. " +
                          ...
                          "한국어로, 이모지 없이, 최대 1문장으로 자연스럽게 말하라.");
            sb.AppendLine("[마피아 게임 말하기 원칙]");
            sb.AppendLine("- 매 발언은 판을 움직여야 한다: 의심, 변호, 반박, 알리바이 요구, 말 바뀜 지적, 투표 압박 중 하나를 하라.");
            sb.AppendLine("- 단서가 없는 초반에는 확신형 몰이를 하지 마라. 먼저 어디 있었는지, 누가 봤는지, 방금 의심의 근거가 뭔지 검증하라.");
            sb.AppendLine("- 의심은 단계적으로 올려라: 확인 질문 → 모순 지적 → 투표 압박. 첫 대화부터 범인 취급하거나 무차별 증거를 강요하지 마라.");
            ...
            sb.AppendLine("- 추리 없이 감상만 말하지 마라. 금지 표현: 무슨 의미죠, 무슨 소리냐, 걱정스럽습니다, 다행입니다, 확인해주실 수 있나요, 명확히 답변드리겠습니다, 필요성을 느낍니다, 정말 궁금해서, AI 같은 소리, 지껄이지 마.");
