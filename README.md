...
6인 마피아 게임을 5명의 로컬 AI(Gemma 3 4B)와 함께 플레이하는 텍스트 심리전 게임.
...
### 2. 로컬 AI 자동 세팅 (Ollama + gemma3:4b)

> ⚠️ AI 모델 가중치(약 3.3GB)는 저장소에 포함되지 않습니다(용량 문제).
...
2. Ollama 서버 실행
3. `gemma3:4b` 모델 다운로드(약 3.3GB) + 워밍업
...
| 모델 | `gemma3:4b` (약 3.3GB, setup 스크립트가 pull) |
| 디스크 | 모델용 약 4GB 여유 |
| RAM | 8GB+ 권장 (4B 모델 구동) |
...
- **모델이 없다고 나옴** → `ollama pull gemma3:4b` 수동 실행.
